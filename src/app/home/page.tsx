...
import { useSummarizePersonality } from "@/app/hooks/useSummarizePersonality";
...
export default function Home() {
  ...
  // Platform is fixed to Instagram only
  ...
  // Don't render content until auth check passes
  ...
  const addWatermark = async (imageUrl: string): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      loadImages.then(() => {
        ...
        const logoY = y - fontSize + (fontSize - logoSize) / 2;
  ...
  const handleRefinePrompt = () => {
    if (!bgDescription.trim()) {
      setAlert({ message: "Please enter some text to refine!", type: "warning" });
      return;
    }
    ...
    summarizePersonality(
      ...
      },
      {
        ...
        onError: (error: any) => {
          setAlert({ message: error.message || "Failed to refine prompt", type: "error" });
        },
      }
    );
  };
    ...
    <div className="app-theme-bg flex items-center justify-center p-4 min-h-screen">
      ...
      <div className={`flex flex-col ${(generatedCaption || generatedImage) ? 'xl:flex-row xl:items-stretch' : 'items-center'} justify-center gap-8 w-full transition-all duration-700`}>
        ...
        <div className="glass-card w-full max-w-lg p-8 shrink-0 animate-in fade-in duration-500 flex flex-col">
          ...
          {/* {!showRemoveBg && <div className="mb-6">...</div>} */}
